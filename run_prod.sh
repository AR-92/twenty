...
# Twenty CRM Production Run Script
# This script runs the Twenty CRM project in production mode
# Usage: ./run_prod.sh
...
verify_setup() {
    ...
    if [ ! -d "$PROJECT_DIR/node_modules" ]; then
        log_error "Dependencies are not installed. Please run setup_prod.sh first."
        exit 1
    fi
    
    if [ ! -f "$PROJECT_DIR/packages/twenty-server/.env" ]; then
        log_error "Server environment file does not exist. Please run setup_prod.sh first."
        exit 1
    fi
    
    if [ ! -f "$PROJECT_DIR/packages/twenty-front/.env" ]; then
        log_error "Frontend environment file does not exist. Please run setup_prod.sh first."
        exit 1
    fi
    
    if [ ! -d "$PROJECT_DIR/packages/twenty-server/dist" ]; then
        log_error "Server application is not built. Please run setup_prod.sh first."
        exit 1
    fi
    
    if [ ! -d "$PROJECT_DIR/packages/twenty-front/dist" ]; then
        log_error "Frontend application is not built. Please run setup_prod.sh first."
        exit 1
    fi
...
check_docker_containers() {
    log_info "Checking if required Docker containers are running..."
    ...
    if [ ! "$(docker ps -q -f name=twenty_pg)" ]; then
        ...
        if [ "$(docker ps -aq -f name=twenty_pg)" ]; then
            ...
            log_error "PostgreSQL container is not available. Please run setup_prod.sh first."
            exit 1
        fi
    fi
    ...
    if [ ! "$(docker ps -q -f name=twenty_redis)" ]; then
        ...
        if [ "$(docker ps -aq -f name=twenty_redis)" ]; then
            ...
            log_error "Redis container is not available. Please run setup_prod.sh first."
            exit 1
        fi
    fi
...
start_prod_application() {
    log_info "Starting Twenty CRM application in production mode..."
    
    cd "$PROJECT_DIR"
    
    # Start backend server in production mode
    log_info "Starting backend server in production mode on port $BACKEND_PORT..."
    ...
    # For production, we typically serve the frontend through a web server
    # Let's use a simple Node.js server to serve the built frontend
    log_info "Starting frontend server in production mode on port $FRONTEND_PORT..."
    ...
    if wait_for_port $FRONTEND_PORT 30; then
        ...
        log_error "Frontend server failed to start on port $FRONTEND_PORT within 30 seconds"
        exit 1
    fi
    
    # Start background worker in production mode
    log_info "Starting background worker in production mode..."
    cd packages/twenty-server
    ...
    echo
    log_success "==========================================="
    log_success "TWENTY CRM PRODUCTION SERVERS ARE RUNNING"
    log_success "==========================================="
    echo
...
main() {
    echo
    log_info "==========================================="
    log_info "RUNNING TWENTY CRM IN PRODUCTION MODE"
    log_info "==========================================="
    echo
